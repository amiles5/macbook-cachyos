...
Singleton {
  ...
  readonly property bool acOnline: (UPower.devices?.values || []).some(d => d && d.type === UPowerDeviceType.LinePower && d.online === true)
  ...
  function isPluggedIn(device) {
    ...
    if (device.state !== undefined) {
      return (device.state === UPowerDeviceState.FullyCharged || device.state === UPowerDeviceState.PendingCharge) && root.acOnline;
    }
    return false;
  }
